function clean2foggy_Cityscapes(variant_abbreviation, left_image_file_names,...
    right_image_file_names, disparity_file_names,...
    camera_parameters_file_names, depth_file_names,...
    instance_level_semantic_ground_truth_file_names, load_depth, save_depth,...
    depth_completion_method, scattering_coefficient_method, beta_parameters,...
    atmospheric_light_estimation, transmittance_model,...
    transmittance_postprocessing, transmittance_postprocessing_parameters,...
...
%CLEAN2FOGGY_CITYSCAPES  Simulate fog for Cityscapes.
%   INPUTS:
%
%   -|variant_abbreviation|: string that indicates the abbreviation of the fog
%    simulation method that is being run, e.g. 'stereoBilateralDual'.
%
...
%
%   -|instance_level_semantic_ground_truth_file_names|: cell array of strings
%    with full paths to annotations of left input images for instance-level
...
%    step.
%
%   -|transmittance_postprocessing_parameters|: parameters used in the
%    |transmittance_postprocessing| function.
%
%   -|fog_optical_model|: handle of the function that implements fog generation
%    from a clean image.
%
...
assert(number_of_images ==...
    length(instance_level_semantic_ground_truth_file_names));
...
for i = 1:number_of_images
    ...
    % Read instance-level semantic labeling provided in Cityscapes if
    % necessary.
    switch variant_abbreviation
        case {'stereoBilateralDual', 'stereogfSemanticRefinement'}
            current_instance_level_semantic_ground_truth_file_name =...
                fullfile(Cityscapes_root_directory,...
                instance_level_semantic_ground_truth_file_names{i});
            instance_gt_labels =...
                imread(current_instance_level_semantic_ground_truth_file_name);
            [label_image, L] =...
                instance_ids_to_label_image_plain_cityscapes(...
                instance_gt_labels);
    ...
    if load_depth_current
        ...
        depth_map = depth_completion_method(input_disparity,...
            current_camera_parameters_file_name, R_left, R_left_uint8,...
            R_right, instance_gt_labels);
    end
    ...
    % Postprocessing of transmittance map. Can stand for no postprocessing.
    t = transmittance_postprocessing(t, R_left, label_image, L,...
        transmittance_postprocessing_parameters);
    ...
    % Simulate fog using the specified fog model and the computed
    ...
    % Suffices specifying the parameter values that were used to generate fog.
